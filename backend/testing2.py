...
def basicinfo(lastname, firstletter):
    ...
    for thing in r['data']['newSearch']['teachers']['edges']:
        print(thing)
        if thing['node']['lastName'].upper() == lastname and thing['node']['firstName'][0].upper() == firstletter:
            big = "tot"
            currval = 0
            for key in thing['node']['mandatoryAttendance']:
                if key != "total" and thing['node']['mandatoryAttendance'][key]>currval:
                    big = key
            return (thing['node']['id'],[thing['node']['avgRating'],big,thing['node']['ratingsDistribution']['total'], thing['node']['wouldTakeAgainPercentRounded']])
        
    return "None"

print(basicinfo('REZK','C'))
